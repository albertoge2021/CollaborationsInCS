...
df["continent"] = df["location"].apply(
...
continent_citations = (
    df.groupby("continent")["citations"].sum().sort_values(ascending=False)
)
continent_mean_citations = (
    df.groupby("continent")["citations"].mean().sort_values(ascending=False)
)
